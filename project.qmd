...
Therefore, our total dataset comprises 57 countries for 52 variables, and spans 1960 to 2026. Of course, for analysis purposes, we will reduce this dataset and use subsets, which will be specified every time.
...
Basic scatter plot for the Phillips curve:
...
data |>
  ...
  select(c("time","country",`Core inflation`,`Unemployment rate`)) |>
  drop_na() |>
  mutate(
    time = as.integer(time)
  ) |>
  ggplot(aes(x=`Core inflation`, y=`Unemployment rate`)) +
  geom_point() +
  facet_grid(~ country)
```
