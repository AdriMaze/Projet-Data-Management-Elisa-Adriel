...
The Phillips curve is a fundamental concept in economics, and the basis of most neo-models, whether it be from classic or more keynesian schools. It was [first observed a negative relationship between unemployment and inflation rate]{.mark}, but as time passes, and accross countries as well, this relationship is [less and less true]{.mark}. Our question concerns the fundamental and exogenous principles behind the [horizontalisation]{.mark} of the Phillips curve. 
...
> ***What drove the variations in the curvature of the Phillips curve since the 1960s? A multi-modal, multi-temporal, cross-country analysis of the impact of underlying regime and policy variables on the relationship between unemployment and inflation***

For this, we have to conduct [multi-modal analysis, *i.e.*, space-time, per- and cross-variables analysis]{.mark} at the same time. The use of the model displayed in [Appendix A](#appendixA) could be appropriate for SVAR analysis, but it has the disadvantages of 1) being hard to grasp for non-economists, especially the self-modified version presented, and 2) being too uselessly heavy in computing terms to draw proper IRFs. Therefore, our final choice sur to use a [self-improved, multi-temporal, cross-country and own- and cross-variable Difference-in-Difference model](#did-model), which will allow us to derive [matrices of underlying parameters]{.mark}, allowing us to derive the different impacts of each parameter over time, as well as their magnitude.  
In a first part, we will focus on recalling today's changing as they are the closest to us and draw some constatations on the curve's relationship. Then, we will divide our temporal analysis in 3 parts, namely before 1990, from 1990 to 2008 and afterwards. We will detail how we think these three periods are the most relevant. Finally, we will focus on certain type of shocks and regime changes which particularly impacted the curve.
...
::: callout-note
On every data source, we took the maximal range of time available, even if all data was `NA` and for all European countries. Please note that European countries does not restrict to solely the European Union, Euro Area or Euro Zone. It, however, does not include neither Turkey, nor Russia. As you can see [here](#basic-info), for WDI, we took a consequent number of variables and therefore you will understand that we will not display all of them here. You can look inside of the data for easy replication of the selection.
:::
...
-   The [OECD](https://www.oecd.org/fr/about.html) is an international institution founded in 1961 to advise
    ...
    warming. It has been collecting and analyzing data for its member
...
Our two main variables are evidently unemployment and inflation, because they are the variables on which we will focus. 

::: callout-tip
## Definitions
The [Consumer Price Index (CPI)]{.mark} is a statistical index that tracks the average change over time in prices paid by households for a fixed representative “basket” of goods and services.  
The [(CPI) inflation rate]{.mark} is the percentage change in CPI over a period (month-to-month, year-to-year), given by:
$$
\pi_t= \frac{CPI_t-CPI_{t-1}}{CPI_{t-1}}\times 100\%
$$
According to OECD definition, an [unemployed individual]{.mark} is a working age individual that is not employed, is available to work and has been actively looking for employment during a reference period.  
The [unemployment rate]{.mark} is therefore defined as the percentage share of unemployed individuals in total labor force, given by:
$$
u_t=\frac{U_t}{L_t}\times 100\%
$$
Where $U_t$ is the total number of unemployed individuals and $L_t$ the total number of individuals in the labor force.
:::

WDI data will be used because of more completeness for the unemployment rate and inflation rate variables. Moreover, we exclude
inflation values above 40% as in [Yamada & Bell
(2012)](https://ejce.liuc.it/18242979201202/182429792012090203.pdf). Let us focus on France:
...
In November 1958, the journal [Economica]{.mark} published an article by A.W.
...
of growth of nominal wages in the United Kingdom, 1861-1957’]{.mark}. Phillips
sought to test the idea that [unemployment and nominal wage growth move
in opposite directions]{.mark}. His intuition was that the labor market is like
...
analysis]{.mark}, thereby providing the missing equation in Keynesian theory.
...
given period described by the aggregate supply-aggregate demand (AD-AS) model of
...
therefore prices, between period $t$ and period $t + 1$. Prices at $t + 1$
will in turn determine demand at $t + 1$, and so on.

This relationship, in a later version, had very [strong implications for
economic policy]{.mark}. Indeed, an extension of the Phillips curve was proposed
...
using WDI data because it is more complete. We still exclude inflation rates above 40%.
