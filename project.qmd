...
countries as well, this relationship is [less and less true]{.mark}. Our
question concerns the fundamental and exogenous principles behind the
[horizontalisation]{.mark} of the Phillips curve.

Indeed, there could be multiple reasons as to why we do not observe this
relationship anymore: the structures of the economies changed, the
globalization had an impact on the fundamental relationship, the policy
regimes' changes contributed to flatten the relationship...
...
(For this, we have to conduct [multi-modal analysis, *i.e.*, space-time,
per- and cross-variables analysis]{.mark} at the same time. The use of
the model displayed in [Appendix A](#appendixA) could be appropriate for
SVAR analysis, but it has the disadvantages of 1) being hard to grasp
for non-economists, especially the self-modified version presented, and
2) being too uselessly heavy in computing terms to draw proper IRFs.
Therefore, our final choice sur to use a [self-improved, multi-temporal,
cross-country and own- and cross-variable Difference-in-Difference
model](#did-model), which will allow us to derive [matrices of
underlying parameters]{.mark}, allowing us to derive the different
impacts of each parameter over time, as well as their magnitude.\
In a first part, we will focus on recalling today's changing as they are
the closest to us and draw some constatations on the curve's
relationship. Then, we will divide our temporal analysis in 3 parts,
namely before 1990, from 1990 to 2008 and afterwards. We will detail how
we think these three periods are the most relevant. Finally, we will
focus on certain type of shocks and regime changes which particularly
impacted the curve.)
...
## Focus variables: unemployment and inflation
